...
public class UserController {
    ...
    public ResponseEntity<Void> deleteUserByIdController(@PathVariable Long id) {
        userService.deleteUser(id);
        return ResponseEntity.noContent().build();
    }

    @PutMapping(value = "/{id}")
    public ResponseEntity<User> insertUserController(@PathVariable Long id, @RequestBody User userRequestBody) {
        return ResponseEntity.ok().body(userService.updateUser(id, userRequestBody));
    }
}
